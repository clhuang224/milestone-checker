...
- [x] 4.1 條件欄位來源改成 `RuleField`（月齡 ＋ 觀察項目），`RuleEditor`/`ConditionEditor` 跟著換型別
- [x] 4.2 條件編輯器支援新增「適用條件」列:對象 × 包含/排除 × 多選項目
- [x] 4.3 UI 文案寫明「排除」＝扣掉這些之後仍有其他錯誤，不是「完全沒有」
- [x] 4.4 元件測試:建立含適用條件的規則、編輯既有規則時能還原成適用條件列
...
- [x] 7.2 e2e 補小切場景（生日由執行日回推，不寫死），確認警示真的跳出來
