...
- [x] 4.1 草擬一小批語言/言語/吞嚥的範例 `FindingDefinition`（觀察項目/評估分數欄位）——`src/app/data/starter-findings.ts`，10 筆，每個類別都有布林+數值型
- [x] 4.2 草擬一小批範例 `Rule`（條件 + 警示文字 + 報告範本片段），每一條都附 `sourceNote`，清楚標示是佔位/示意用——`src/app/data/starter-rules.ts`，4 條，涵蓋純布林組合、數值門檻、布林+數值混合三種條件型態;另外寫了 `starter-content.spec.ts` 驗證 id 不重複、規則只會引用實際存在的觀察項目、每筆都有 `sourceNote`
