...
順序就是執行順序。第 1 節不依賴任何未決問題，可以先做;第 2、3 節卡在 Q2–Q4;第 6 節卡在 Q8／Q9。「聽力分左右耳」與「轉介規則維持原樣」是已定案的決定，見 `proposal.md` 的〈已定案〉。
...
- [ ] 4.5 （需 Q8）元件測試:用編輯器建出「母語含台灣華語 AND 兩耳聽力正常 AND 有聲調錯誤」（實際幾列依 Q8 的答案），存檔後重新開啟能還原成對應的條件列而不是原始 JSON
...
- [ ] 6.2 （需 Q8／Q9）若要出貨:`data/starter-rules.ts` 新增這條規則（「聽力正常」依 Q8 的答案寫成 AND 或 OR），並把 `storage.ts` **所有** key 一起升到 `:v8`;`starter-content.spec.ts` 補斷言
