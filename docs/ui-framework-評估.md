...
開發者說「我不喜歡 element prime vuetify mui」。這四個裡面**只有 PrimeNG 是 Angular**：Element Plus 是 Vue，Vuetify 是 Vue，MUI 是 React。所以這不是技術相容性的意見，是**美學上的**：
...
5. **開發者說「我覺得現在的 UI 很棒」。** 這句話是評估的基準線，不是客套。任何一個框架方案的第一步都是**先讓現在的長相變差**，然後花工把它調回來——而且調回來之後，多的是一個依賴。
